...
elseif ($module=='download' AND $act=='input'){
  ...
  $acak           = rand(000,999);
  $nama_web       = 'mystudio';
  $nama_file      = $nama_web.$acak.$nama_file_asli;
  $tgl            = date("Y-m-d");
...
elseif ($module=='download' AND $act=='update'){
  ...
  if (!empty($lokasi_file))
  {
    ...
    {
      $tampil=mysql_query("SELECT * FROM download WHERE id_download='$_POST[id]'");
      $cek=mysql_fetch_array($tampil);
      if($cek[nama_file] != '')
      {
        unlink("../../../files/$cek[nama_file]");
      }
      ...
      UploadFile($nama_file);
      mysql_query("UPDATE download SET tanggal = '$tgl', 
                                        judul     = '$_POST[judul]',
                                        nama_file    = '$nama_file'   
