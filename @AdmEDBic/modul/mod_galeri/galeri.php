...
<!-- /TinyMCE -->
		...
		<article class="module width_full">
			<header><h3 class="tabs_involved">Daftar Galeri Foto</h3>
		...
		<article class="module width_full">
			<header><h3>Tambahkan Galeri Foto</h3></header>
			<form method='POST' enctype='multipart/form-data' action='<?php echo "$aksi"; ?>?module=galeri&act=insertnew'>
				<div class="module_content">
						
						<table>
							<tr><td style="width:20%;"><label>Album</label></td> <td style="width:61%; margin-bottom:8px;">

							<select name="id_album">
								<?php
								$sql="select * from album";
								$result=mysql_query($sql);
								echo "<option value=''>-- Pilihan Album --</option>";
								while($data=mysql_fetch_array($result)){
									echo "<option value='$data[id_album]'>".$data[nama]."</option>";
								}
								?>	
							</select>
						...
						<table>
							<tr><td style="width:20%;"><label>Judul Foto</label></td> <td style="width:50%; margin-bottom:8px;"><input style="width:100%; margin-bottom:8px;" name="nama" type="text" ></td></tr>
						</table>
		...
		<article class="module width_full">
			...
			<form method='POST' enctype='multipart/form-data' action='<?php echo "$aksi"; ?>?module=galeri&act=update'>
				...
				<div class="module_content">
						...
						while($data=mysql_fetch_array($result)){
						if($data[id_album] == $r[id_album]){
								echo "<option value='$data[id_album]' selected>".$data[nama]."</option>";
							}else{
								echo "<option value='$data[id_album]'>".$data[nama]."</option>";
						}
						}
						?>	
							...
							<table>
							<tr><td style="width:20%;"><label>Judul Foto</label></td> <td style="width:50%; margin-bottom:8px;"><input style="width:100%; margin-bottom:8px;" name="nama" type="text" value="<?php echo $r[nama] ?>" ></td></tr>
						</table>
						<fieldset style="float:left; width:30%; margin-right: 3%;"> <!-- to make two field float next to one another, adjust values accordingly -->
