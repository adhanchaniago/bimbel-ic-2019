...
$(document).ready( function () {
     var oTable = $('#tbprogram').dataTable( {
                    "bJQueryUI": true,
...
<style>.ui-widget-header{background:none;border:none;}</style>
		...
		<?php
		$aksi="modul/mod_tbprogram/aksi_tbprogram.php";
		switch($_GET[act]){
			default:
		?>
		<article style="max-width:660px;" class="module width_3_quarter">
			<header><h3 class="tabs_involved">Program</h3>
				
			</header>

			<table class='display' id='tbprogram'>
			<thead> 
				<tr>  
    				<th>No</th>
    				<th>Program Name</th>
		...
		<article style="min-width:260px;" class="module width_quarter">
			 <header><h3>Post Program</h3></header>
			 <form method='POST' enctype='multipart/form-data' action='modul/mod_tbprogram/aksi_tbprogram.php?module=tbprogram&act=insertnew'>
		...
		<?php break; 
		case"edit":
			$category = mysql_query("SELECT * FROM tb_program WHERE program_id='$_GET[id]'");
				$g=mysql_fetch_array($category);
				//echo $g['bimbingan_id'];; exit;
		...
		<article class="module width_quarter" style="width: 50%;">
			 ...
			 <form method='POST' enctype='multipart/form-data' action='modul/mod_tbprogram/aksi_tbprogram.php?module=tbprogram&act=update'>
